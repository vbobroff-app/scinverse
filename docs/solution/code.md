...
- Каталог/группировка (read-model): `InstrumentQuery` (фильтры `q`/`board`/`secType`/`category`/
  `onlyRecording`/`nonEmpty`/`instrumentIds`/`exchanges`/`underlyingCode`/`expiration` +
  `limit`/`offset`; phase 7d — плашки «Инструмент/Выбор/Биржи») → `InstrumentCatalogPage`
...
- `CoverageStore : ICoverageStore` — `coverage_segment`: `Open` (SELECT активного → иначе INSERT
  ...
  `Close` (`UPDATE … ended_at, status`), `RecoverOpenSegments` (phase 7h: на старте закрывает
  осиротевшие открытые сегменты `status='interrupted'` по времени последней сделки).
...
- `TradeActivityStore : ITradeActivityStore` (phase 7g) — присутствие сделок по бакетам
  (`time_bucket` на лету + кэш закрытых дней); закрытые дни лениво материализуются, текущий день
  считается на лету. Возвращает только непустые бакеты.
- `CaptureLivenessStore : ICaptureLivenessStore` (phase 7h) — живость захвата: `Heartbeat`
  (открыть/продлить `to_ts` в пределах `maxGap`, иначе новый интервал), `Close`, `Query`,
  `RecoverOpenIntervals` (на старте закрывает осиротевшие).
...
- `V008__trade_activity_bucket.sql` — кэш присутствия сделок (phase 7g): `trade_activity_bucket`
  (непустые бакеты `(instrument, source, bucket_size, bucket_ts)`) + `trade_activity_computed`
  (маркеры посчитанных закрытых дней, в т.ч. пустых).
- `V009__coverage_interrupted_status.sql` — статусы закрытия сегмента `disconnected`/`interrupted`
  в `ck_coverage_status` (phase 7h — recovery/обрывы).
- `V010__capture_liveness.sql` — `capture_liveness` (phase 7h): компактные интервалы живости захвата
  `(source_id, from_ts, to_ts, open)` + уникальный индекс «один открытый интервал на source».
- `V011__capture_liveness_reason.sql` — `close_reason` (`stopped|server_down|ping_failed|interrupted`);
  журнал разрывов — производное (`QueryGapsAsync`), отдельной таблицы нет.
...
- `TradeActivityStore` (phase 7g) — кэш закрытых дней + расчёт живого хвоста на лету; досып сделки в
  уже посчитанный закрытый день не меняет ответ (кэш авторитетен);
- `CoverageStore.RecoverOpenSegments` (phase 7h) — закрытие осиротевшего по последней сделке / по
  `started_at` для пустого; закрытые сегменты не трогает;
- `CaptureLivenessStore` (phase 7h) — хартбит продлевает интервал в пределах `maxGap` и рвёт на два при
  большем разрыве; `Close`/`Recover` закрывают открытый; повторное открытие после закрытия.
...
- Компоненты: `InstrumentPicker` (дерево инструментов + дорожки + footer), `CoverageTrack`
  ...
  (панель таймфреймов), `FilterBar`/`FilterChips` (динамические плашки-фильтры каталога, phase 7d), `Button`, `StatusDot`,
  `ThemeToggle`.
