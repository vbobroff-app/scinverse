...
- **DTO** — `InstrumentDto` (+ деривативные поля `underlyingCode`/`optionType`/`strike`/
  ...
  `ConnectionDto`, запросы `InstrumentQueryParams`/`StartRecordingRequest`/
  `UpsertConnectionRequest`/`ConnectionCredentialsRequest`.
...
- `OhsEndpoints` — маршруты `/api/*`: `instruments` (пагинация + фильтры, phase 7),
  `instruments/groups` (дерево деривативов, phase 6c), sources, coverage(+gaps), recordings,
