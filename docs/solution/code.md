...
- **DTO** — `InstrumentDto`, `SourceDto`, `CoverageSegmentDto` (+ `GapDto`), `RecordingDto`,
  `ConnectionDto`, запросы `StartRecordingRequest`/`UpsertConnectionRequest`/`ConnectionCredentialsRequest`.
...
- `OhsEndpoints` — маршруты `/api/*` (instruments, sources, coverage(+gaps), recordings,
...
Testcontainers-БД, строка подключения переопределяется) и бьёт рукописным клиентом `OhsApiClient`
(реализация `IOhsApi`):
reference-данные без секретов, полный цикл записи (connect → start → рост `trade_count` → stop →
закрытый сегмент), `/ws` присылает `coverageExtended`. Требует Docker.
