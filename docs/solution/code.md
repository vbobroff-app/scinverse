...
- `TradeNormalizer` — `TryNormalize(TradeEvent, out TradeRecord)`: переводит `TradeEvent`
  (цена в деньгах) в `TradeRecord` (`InstrumentId` + `PriceTicks` через `instrument.ToTicks`).
  Сделки по незарегистрированному инструменту отбрасываются (`false`), а не роняют поток.
...
3. `INSERT INTO md_trade SELECT … FROM _stage_trade ON CONFLICT (instrument_id, trade_no, ts)
   DO NOTHING` — переливка с **дедупликацией** по первичному ключу.
...
запиненный образ), прогоняет реальные миграции (DbUp) и проверяет `TimescaleTradeWriter`:
запись+чтение, идемпотентность (дедуп по PK), `NULL open_interest`, пустой батч. Требует
запущенный Docker.
