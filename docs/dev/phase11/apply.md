...
| Событие | Метод хаба | severity / status | Источник |
| ------- | ---------- | ----------------- | -------- |
| потеря связи (`Down`/`Error` от коннектора) | `Open` | `error` / active | `ConnectionManager.HandleLinkStateAsync` |
| попытка реконнекта | `Progress` | `warning` / underway | `ConnectionSupervisor` |
| восстановление | `Resolve` | `ok` / resolved | `ConnectionManager` |
| **connect оператором** (REST) | `Publish` | `info`, `sourceType=user` | эндпоинт `/connect` |
| **connect не удался** | `Publish` | `error`, `sourceType=user` | эндпоинт `/connect` |
| **disconnect оператором** (REST) | `Publish` + `Resolve` | `info`, `sourceType=user` | эндпоинт `/disconnect` |
