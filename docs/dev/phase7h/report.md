...
| # | Задача | Статус | Комментарий |
|---|--------|--------|-------------|
| 7h.8a | data: `V020 link_liveness` + `ILinkLivenessStore`/`LinkLivenessStore` + DI | DONE | зеркало `capture_liveness` + `disconnected`; тесты `LinkLivenessStoreTests` (7) зелёные |
| 7h.8b | writer: link-heartbeat/close в `ConnectionManager` + keepalive 24/7 + recovery | DONE | connect/Live→heartbeat, Down/Error→server_down, Disconnect→disconnected; keepalive в `LivenessProbe.TickAsync` без гейта; recovery в `Program.cs` |
