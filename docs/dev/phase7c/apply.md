...
- В ответе борда (`…/boards/{board}/securities`) на контракт есть **`ASSETCODE`** (код базового
  актива) и `SECTYPE`. **Готовой категории нет** — `group` у всех фьючерсов = `futures_forts`.
- Категория выводится из класса базового актива двумя путями:
  1. **Спот-активы** (акции/индексы/валюта, торгуемые на споте) резолвятся через
     `GET /iss/securities.json?q=<ASSETCODE>` → поле **`group`**: `stock_shares`→Акции,
     `stock_index`→Индексы, `currency_selt`→Валюта (значения см. `/iss/securitygroups`,
     `/iss/securitytypes`). Это авто-классификация «из ISS».
  2. **Товары, ставки, иностранные/крипто-индексы** в спот не резолвятся (нет базовой бумаги) →
     класс берётся из спецификации кодов (`s205`), закрепляется в курируемой части справочника.
...
2. Каждый код классифицируем: сначала **сид-карта `s205`** (`FuturesAssetTaxonomy.Seed` — валюта/
   ставки/товары/крипто- и спец-индексы), иначе — резолв спот-актива
   `GET /iss/securities.json?q=<ASSETCODE>` → маппинг `group` (акции/обычные индексы), иначе `other`.
