...
- Описание календарей (PDF): [«Все рынки (ФР, ВР, СР) — calendars»](https://www.moex.com/files/4rkd3yjkghfhqz4h7g8rewetx4)
  — описывает платный `/iss/calendars`; **не** путь, которым пользуемся в MVP.
- **apimoex** (Python-клиент к ISS, Unlicense) — [репозиторий](https://github.com/WLM1ke/apimoex) ·
  [справочник API](https://wlm1ke.github.io/apimoex/build/html/api.html). Независимое человекочитаемое
  описание запросов: часто явнее официального Reference, где календарь «спрятан» под описанием
  движка (`reference/397`), а не как раздел «calendars». **История находки `timetable`/`dailytable`
  и что ещё полезно взять из apimoex — §2c.**
...
> ✅ **РАБОЧИЙ БЕСПЛАТНЫЙ ИСТОЧНИК (проверено на живом ISS 2026-07): `/iss/engines/{engine}.json`.**
> Машиночитаемый календарь MOEX доступен **без подписки** через движок (НЕ через платный
> `/iss/calendars`). Как нашли и почему это не «из официальной доки calendars» — §2c (apimoex).
> Две таблицы (фильтр `iss.only=timetable|dailytable`, `iss.meta=off`, `lang=ru`):
>
> - **`timetable`** — недельное расписание: `week_day (1=пн…7=вс), is_work_day, start_time, stop_time`.
>   `stock` → все 7 дней рабочие, `06:20–23:59:59`; `futures` → все 7 дней, `07:00–23:59:59`
>   (подтверждает торги в выходные / ДСВД).
> - **`dailytable`** — исключения по датам (перекрывают недельное): `date, is_work_day, start_time,
>   stop_time`. `is_work_day=0` — праздник/неторговый; `is_work_day=1` — перенос (рабочий день).
>   Есть **будущие** даты (напр. `2026-05-01`→0, `2026-06-12`→1). Фильтр `?from=YYYY-MM-DD&till=…`.
>
> **Алгоритм «дата X торговая?»:** есть строка в `dailytable(X)` → берём её `is_work_day`/часы; иначе —
> `timetable[weekday(X)]`. Часы `start/stop_time` — **внешняя граница дня** (для оси Ганта). Тонкая
> структура сессии (утро/основная/вечер/аукционы/ДСВД-окно) — из s1167 (§3), она меняется редко.
> Примеры: `GET /iss/engines/futures.json?iss.only=dailytable&from=2026-01-01&till=2026-12-31&iss.meta=off`,
> `GET /iss/engines/stock.json?iss.only=timetable&iss.meta=off&lang=ru`.

> ⚠️ **`/iss/calendars/*` — ЗА ПОДПИСКОЙ** (проверено 2026-07): `/iss/calendars.json`,
> `/iss/calendars/futures`, `…/session` → «Информация доступна только подписчикам». Раздел ниже про
> `off_days`/`session_schedule` оставлен как справка на случай подписки; для free-доступа он **не нужен**
> — используем `/iss/engines/{engine}.json` (выше) и/или историю (§2a).

Таблица `off_days` — исключения к офиц. праздникам + рабочие выходные, **по рынкам** (⚠️ подписка).
...
**Бесплатный авторитетный календарь — интерактивная страница MOEX:**
[moex.com/msn/ru-se-calendar](https://www.moex.com/msn/ru-se-calendar) — торговый календарь ФР (рабочие/
выходные/сокращённые дни, включая **рабочие выходные и дни ДСВД**). Это официальный публичный
HTML-источник; машиночитаемый аналог — `/iss/engines/{engine}.json` (`timetable`/`dailytable`,
см. начало §2 и историю находки в §2c).

## 2a. Торговый календарь БЕСПЛАТНО — из истории торгов (рабочий путь)

История EOD (итоги торгов) доступна **без подписки**. Множество дат с торгами = торговый календарь
(праздники, сокращённые и рабочие выходные/ДСВД уже учтены — если в этот день торговали, дата есть).

- **Доступный диапазон истории** (границы, чтобы знать откуда тянуть):
  `GET /iss/history/engines/{engine}/markets/{market}/dates.json?iss.meta=off`
  → таблица `dates` `{from, till}`. Проверено: `futures/forts` → `2001-09-07 … 2026-07-10`.
- **Торговые даты рынка** (по ликвидному эталону, только колонка даты):
  `GET /iss/history/engines/stock/markets/shares/boards/TQBR/securities/SBER.json`
  `?from=…&till=…&iss.only=history&iss.meta=off&history.columns=TRADEDATE,CLOSE` → `history[].TRADEDATE`.
  - Эталон ФР — ликвидная акция (`SBER`/`TQBR`); СР — фьючерс/индекс FORTS; ВР — свой ликвидный тикер.
  - ⚠️ **НЕ брать индекс `IMOEX`** как эталон: индекс в выходные не рассчитывается, поэтому его
    история не содержит суббот/воскресений и создаёт ложное «выходные — неторговые».
- ⚠️ **Ограничение по выходным (ДСВД).** На MOEX с 2025 работает **сессия выходного дня** (акции
  торгуются и в сб/вс). Но EOD-таблица `history` **не отражает** выходные как отдельные `TRADEDATE`
  (проверено: `SBER/TQBR` за 04–05.07.2026 отсутствуют — итоги ДСВД уходят в клиринг ближайшего буднего
  дня). Значит по истории надёжно виден только **будний** календарь; торговые **выходные** из неё не
  извлечь. Их даёт либо платный `Машиночитаемый календарь` (§2b), либо наши **собственные** записанные
  `md_trade` (для Ганта этого достаточно — мы и так пишем данные, в т.ч. в выходные).
- **Ограничение по будущему.** История — ретроспектива: знает прошлое/сегодня, но **не знает будущие
  праздники**. Для «предсказать завтра» — fallback на дат-версионные константы `MoexSchedule`.
- Пагинация истории — курсор `history.cursor` (`INDEX/TOTAL/PAGESIZE`), тянем страницами по 100.

## 2b. Платная подписка ISS «Машиночитаемый календарь» (для справки)

Авторитетные `off_days` + `session_schedule` продаются как услуга **«Машиночитаемый календарь»**
(прайс MOEX, приказ МБ-П-2025-3358 от 01.09.2025, §25). Доступ — по **Договору ИТО**, авторизация
**MOEX Passport (юрлицо)**; заявки/тарифы — `datasales@nsd.ru`;
оформление — [iss.moex.com/iss/docs/corpinfo/v1/service_subscription](https://iss.moex.com/iss/docs/corpinfo/v1/service_subscription/).

**Тариф (руб./мес., с 01.01.2026):**

| Назначение | Один рынок (ФР/ВР/СР) | Все три рынка |
| ---------- | --------------------- | ------------- |
| Индивидуальное использование (без распространения) | 20 000 ₽ | 50 000 ₽ |
| **Демонстрация на сайтах/в приложениях** (наш кейс) | **22 000 ₽** | **55 000 ₽** |
| Распространение | 50 000 ₽ фикс. + переменная (по-клиентски) | 40 000 ₽ + переменная |

> До 31.12.2025 тариф был символическим (~200–550 ₽/мес), но с 2026 вырос в ~100 раз. В абонплату
> входит до 5 логинов. **Вывод:** платная подписка НЕ нужна — авторитетный машиночитаемый календарь
> (даты + внешние часы, вкл. будущие праздники и ДСВД) доступен бесплатно через
> `/iss/engines/{engine}.json` (`timetable`/`dailytable`, см. начало §2); тонкие окна сессий — из s1167 (§3).

## 2c. Как нашли бесплатный календарь (и зачем смотреть apimoex)

В официальной доке MOEX «календарь» ассоциирован с `/iss/calendars` и PDF «Все рынки — calendars».
В ISS Reference нет отдельного раздела «торговый календарь бесплатно». Путь, которым пользуемся,
нашли **не из этой доки**, а обходным путём — зафиксировано здесь, чтобы не потерять.

### Хронология (2026-07)

1. **Платный путь отпал.** Живой ISS ответил на `/iss/calendars/*` (и `off_days`, и `session`,
   и корень) — «Информация доступна только подписчикам». Путь из PDF/раннего plan — закрыт.
2. **История `TRADEDATE` — неполная.** Бесплатный `history` даёт будние торговые дни, но **не**
   отражает ДСВД-выходные отдельными датами (итоги уходят в клиринг ближайшего будня); индексы
   вроде `IMOEX` в выходные вообще не считаются — ложный эталон.
3. **Публичные HTML.** Пользователь указал открытые страницы MOEX:
   [ru-se-calendar](https://www.moex.com/msn/ru-se-calendar) и [s1167](https://www.moex.com/s1167)
   (расписание по режимам + XLSX) — данные есть бесплатно, но не как `/iss/calendars`.
4. **Находка в apimoex.** В справочнике Python-клиента [apimoex](https://github.com/WLM1ke/apimoex)
   функция [`get_engine`](https://wlm1ke.github.io/apimoex/build/html/api.html#apimoex.get_engine)
   явно описывает: `GET /iss/engines/{engine}.json` содержит таблицы `engine` / **`timetable`** /
   **`dailytable`**, и `dailytable` назван **«источник торгового календаря MOEX»**. Указан и
   официальный номер запроса — [iss.moex.com/iss/reference/397](https://iss.moex.com/iss/reference/397)
   (описание движка; календарь «спрятан» под ним, не под `/iss/calendars`).
5. **Проверка на живом ISS.** Запрос без авторизации отдал обе таблицы (вкл. будущие праздники/
   переносы 2026). В том же ответе лежит и `session_schedule` на текущий день — для MVP тонкие окна
   берём из s1167 (стабильнее регламент), внешние границы дня — из `timetable`/`dailytable`.

**Вывод:** официальный Reference **содержит** endpoint (`reference/397`), но не позиционирует его
как замену платному «Машиночитаемому календарю». Человекочитаемое указание «это и есть календарь»
нашли в **apimoex**; живой ISS подтвердил бесплатность. Дальше — держать apimoex как карту
«что ещё спрятано в ISS, но уже обкатано сообществом».

### Пример из apimoex (как описан запрос)
...
with requests.Session() as session:
    # Недельное правило (пн–вс → is_work_day, start/stop_time)
    weekly = apimoex.get_engine(session, engine='futures', table='timetable')
    # Исключения по датам (праздники / переносы / сокращённые)
    daily = apimoex.get_engine(session, engine='futures', table='dailytable')
```

Эквивалент без библиотеки (то, что зовёт наш `IssExchangeCatalog.GetEngineCalendarAsync`):

```
GET https://iss.moex.com/iss/engines/futures.json?iss.only=timetable,dailytable&iss.meta=off&lang=ru
```

### Что ещё полезно смотреть в apimoex (кандидаты на будущее)

Мы **не** тянем Python-зависимость в Host — берём только знание endpoint’ов. Уже используем
аналоги `get_engines` / структура бордов / `securities` / описание инструмента (`GROUPTYPE`).
Из [справочника apimoex](https://wlm1ke.github.io/apimoex/build/html/api.html) потенциально:

| apimoex | ISS | Зачем нам |
| ------- | --- | --------- |
| `get_engine` (`timetable`/`dailytable`) | `/iss/engines/{engine}` · [ref/397](https://iss.moex.com/iss/reference/397) | **Уже в MVP 7c** — календарь + внешние часы |
| `get_reference` | `/iss/index` / placeholders | Справочники `boards`, `securitytypes`, `securitygroups`… |
| `find_securities` | `/iss/securities.json?q=` | Поиск по ISIN/регномеру/части тикера (история тикеров эмитента) |
| `find_security_description` | `/iss/securities/{secid}` | Спека инструмента (дата старта торгов и др.) — рядом с нашим `GROUPTYPE` |
| `get_board_securities` | `…/boards/{board}/securities` | **Уже в 7c.5** — список инструментов борда |
| `get_board_dates` / `get_board_history` / `get_market_history` | `…/history/…` | Ретроспектива торговых дней / EOD (фолбэк §2a; не для ДСВД) |
| `get_market_candles` / `get_board_candles` (+ `*_candle_borders`) | `…/candles` | OHLCV-свечи (не MVP админки; может пригодиться cold-path/аналитике) |
| `get_index_tickers` | analytics индекса | Состав индекса на дату |

Плюс универсальный `ISSClient` в apimoex — шаблон пагинации курсором (`start`/`INDEX`/`TOTAL`);
у нас свой парсер `IssTable`, но паттерн курсора тот же, что у ISS history.

Периодически сверять apimoex при расширении ISS-прокси: библиотека короче официального
Reference и уже отфильтровала рабочее.

## 3. Внутридневное расписание сессий (session_schedule)

> ⚠️ `/iss/calendars/{market}/session` (JSON API) — **за подпиской**. НО авторитетное **расписание
> по режимам публикуется БЕСПЛАТНО** на сайте: **[moex.com/s1167](https://www.moex.com/s1167)** —
> HTML-таблицы + XLSX «Расписание торгов по режимам, виды заявок, коды расчетов» (~112 Кб).
...
| ------ | ---------------------------- | --------------- |
| Утренняя (доп.) | 06:50:00 — 09:49:59 (АО 06:50–06:59, ТП с 07:00) | — |
| Основная | 09:50:00 — 18:59:30 (ТП до 18:54:59, АЗ 18:55–18:59:30) | 09:50:00 — 18:59:59 |
| Вечерняя (доп.) | 19:00:01 — 23:49:59 | — |

Т.е. граница дня будни ≈ `06:50…23:49:59`, выходной день (ДСВД) ≈ `09:50…18:59:59`. Облигации/РЕПО и
СР — свои таблицы (там же на s1167 / отдельная страница СР). Аукционы/перерывы — при желании детальнее.

Ниже — исходная (платная) `session_schedule` через ISS, оставлена как справка на случай подписки.

Таблица `session_schedule` — периоды торгов **на текущий торговый день** по рынку/бордам (⚠️ подписка).
...
По уведомлению биржи (QUIK-сообщение / [moex.com/n101980](https://www.moex.com/n101980))
**с 14 июля 2026** на срочном рынке действует новый регламент — **внешняя граница дня
выравнивается с фондовым рынком** (`06:50–23:50` в рабочие дни).

| Период | До 13.07.2026 (МСК) | С 14.07.2026 (МСК) |
| ------ | ------------------- | ------------------ |
| Аукцион открытия | `08:50–09:00` | `06:50–07:00` |
| Утренняя сессия | `09:00–10:00` | `07:00–10:00` |
| Основная сессия | `10:00–19:00` | `10:00–19:00` (без изменений) |
| Вечерняя сессия | `19:00–23:50` | `19:00–23:50` (без изменений) |
| **Внешняя граница дня** | **`08:50–23:50`** | **`06:50–23:50`** (= как ФР) |

**Важно (внутренние зоны ≠ ФР):** выравнивается только **старт/конец дня**. Разбивка
утро/основная/вечер у СР и ФР **разная**:

| | ФР (акции, s1167) | СР/FORTS с 14.07 |
|--|-------------------|------------------|
| Утро | `06:50–09:50` | `07:00–10:00` |
| Основная | `09:50–~19:00` | `10:00–19:00` |
| Вечер | `19:00–23:50` | `19:00–23:50` |

- В утреннюю сессию СР доступны сделки **со всеми инструментами, кроме опционов на валютные пары**.
- ДСВД (сб/вс) пока без изменений: ≈ `09:50–19:00` (с 18.07.2026 валютные фьючерсы тоже в ДСВД —
  см. n101980).
- **13.07.2026** живой ISS `session_schedule` ещё отдавал старый регламент (утро с 09:00) —
  переключение на стороне биржи со следующего торгового дня.

**Следствия для Scinverse:**

- **Не хардкодим** новый регламент в `MoexSchedule` / `moexSession.ts`. Фолбэк остаётся
  `08:50–23:50` / ДСВД `09:50–19:00` — намеренно, чтобы **пережить 14.07.2026 на живом ISS** и
  увидеть, как система подхватывает внешнее изменение (`timetable` / `session_schedule` /
  `dailytable`).
- Авторитетный путь — `IMarketCalendar` ← `GetEngineCalendarAsync` (кэш in-memory TTL ~12ч).
  После обновления ISS может понадобиться дождаться истечения TTL или рестарт Host, иначе
  отдастся устаревшая копия — это как раз то, что стоит проверить 14.07.
- Тонкая разбивка `[утро|основная|вечер]` для зон Ганта — отдельный слой; источник —
  `session_schedule` движка / s1167, не `timetable`.
