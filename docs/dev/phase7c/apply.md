...
> `/iss/calendars`). Две таблицы (фильтр `iss.only=timetable|dailytable`, `iss.meta=off`, `lang=ru`):
>
...
выходные/сокращённые дни, включая **рабочие выходные и дни ДСВД**). Это официальный публичный источник
(в отличие от платного JSON `/iss/calendars`). Нужно найти его машиночитаемый фид (JSON/iCal) либо
парсить страницу; см. §2a как бесплатный API-путь для будней.
