...
**Текущий статус:** `DONE` (MVP Stage 1) — 2026-08-04.
Уровень 3 (карточка провайдера) — готов и принят как MVP.
Уровни 1/2 исходного плана: **не делаем в Stage 1** (см. §Закрытие ниже).
...
## Закрытие MVP (2026-08-04)

Исходная IA phase 7 — три уровня. Решение для Stage 1:

| Уровень | Что было в плане | Решение |
| ------- | ---------------- | ------- |
| **3** | Карточка провайдера: инструменты, Start/Stop, Гант | **DONE** — это и есть MVP админки |
| **2** | Обзорный Гант по всем провайдерам сразу | **DEFERRED** → после WebGL (phase 12); не MVP |
| **1** | Отдельный экран/навигация «выбор биржи» (MOEX / заглушки) | **WONT для MVP** — оставляем текущий UX; куски уже есть в 7c/7e/рейле |

Покрыто другими подфазами (не блокирует закрытие 7): подключения — 7e; окно Ганта/таймфреймы — 7b/7f;
фильтры каталога — 7d; «Биржи → Структура» — 7c.
...
Фаза 7 **закрыта** для Stage 1: ур.3 = рабочая админка; ур.1 WONT MVP; общий Гант (ур.2) —
после phase 12. Дерево снято ранее (фильтры 7d).
