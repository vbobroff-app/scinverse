...
### 8.1. Реализовано: детект недоступности бэка (mock optimistic, 7j.20)

Бэк не может сообщить о **собственном** простое, пока лежит → детектит **клиент**, оптимистично показывает **одиночное fatal**, а по реконнекту POST'ит его в NC **задним числом**.

- **Детект (фронт)**: `createLiveStream` (`live.ts`) — `closeObserver` шлёт `onDrop` при закрытии WS **после** однажды живой связи (первичный неуспех коннекта простоем не считаем). `retry` пере-подписывается каждые 2 c → `onDrop` дедупится в `OhsStore` (первый дроп фиксирует начало).
- **Grace ~6 c** (`BACKEND_OUTAGE_GRACE_MS`): глушим тривиальные блипы (Vite HMR / быстрый рестарт). Если связь не вернулась за grace — оптимистично показываем fatal (`openBackendOutage`, `ts` = момент дропа по часам клиента).
- **Одиночное fatal**: `critical` под **per-outage `correlationId`** (`ohs.backend.outage:<startMs>`). На реконнекте `resolveBackendOutage` обновляет **ту же строку на месте** (I2 upsert: тот же `correlationId`+`code`+`status`) на «Сервер OHS был недоступен · N с» — без второй строки, без зелёного близнеца.
- **mock-POST**: `POST /api/notifications` (`OhsEndpoints`) → `NotificationHub.Ingest(...)` пишет событие **вербатим** (клиентский `id` + backdated `ts`) в ring-buffer/аудит-лог и broadcast'ит по WS. Echo POST'а приходит с тем же `id` → дедуп на шине (без дубля). Так строка **переживает reload** (гидрируется из БД).
- **Точность начала простоя**: по часам клиента, ± один цикл WS-retry (~2 c). Более точный источник — `link_liveness.to_ts` осиротевшего интервала (backend-side, `RecoverOpenIntervalsAsync` помечает краш `interrupted`); это будущий backend-side путь, комплементарный клиентскому.
