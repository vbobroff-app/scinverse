...
## 8. Будущее: NC как внешний сервис (набросок)

Направление (абстрактно, детали — когда наберётся больше информации):

- **NC выносится в отдельный внешний сервис**: своё облако, своя БД уведомлений, поставка как **MFE-компонент**. Единая шина уведомлений для всех сервисов микросервисной архитектуры (не только OHS).
- **Сейчас — mock-behaviour, optimistic pattern**: клиент публикует уведомление оптимистично локально; тот же контракт позже уходит POST'ом во внешнюю NC. Меняется только транспорт, UX неизменен.
- **Отдельный mock-POST с backdated-записью**: умеет писать уведомление в БД NC с **прошедшей датой**. Мотив — при недоступности бэка POST уходит только по восстановлении связи, а само событие (отключение) произошло раньше → запись должна лечь **задним числом** (реальное время события, не время доставки).
- Боли/крайние случаи (дедуп между клиентами, авторитетность client-события, close инцидента, durability) — прорабатываем отдельно позже.

### 8.1. Реализовано: инцидент недоступности бэка (mock optimistic, 7j.20)

Бэк не может сообщить о **собственном** простое, пока лежит → детектит **клиент** и **ведёт инцидент сам** (в отличие от инцидентов связи, где оркестратор — `NotificationHub` на бэке; тут бэк мёртв, владелец — клиент). Стек фаз (единый `correlationId` per-outage):

| фаза | severity · status | code | сообщение |
|---|---|---|---|
| open | `critical` · active | `backend.unavailable` | Сервер OHS недоступен, жду восстановления |
...
- **Граница инцидента** — только доступность **client↔backend**. Здоровье линка к бирже — отдельная нить (инциденты связи). Поэтому «штатно» = бэк отвечает и состояние ре-синхронизировано, а **не** «коннектор подключён» (иначе вне окна расписания/в выходной ok не наступил бы).
- **Детект (фронт)**: `createLiveStream` (`live.ts`) — `closeObserver → onDrop` при закрытии WS **после** однажды живой связи (первичный неуспех коннекта простоем не считаем). `retry` пере-подписывается ~2 c → `onDrop` дедупится в `OhsStore`.
- **Стейт-машина (`OhsStore`)**: `grace` (~6 c, глушит блипы HMR/быстрого рестарта) → `open` (fatal) + интервал `error`-тиков (5 c) → на re-open WS + refresh-батч → `warning` → **settle ~5 c** без нового дропа → `resolve` (ok). Повторный дроп во время warning/settle = бэк снова упал (crash-loop) → **тот же** инцидент возвращается в `error` (без нового fatal), `outageStart` сохраняется.
- **Живые тики (I2 upsert)**: повторы `error`/underway с тем же code обновляют строку **на месте** («· 5 с» → «· 10 с»); каждый тик — свежий `id` (иначе дедуп по id отбросит).
- **mock-POST**: `POST /api/notifications` (`OhsEndpoints` → `NotificationHub.Ingest`) пишет событие **вербатим** (клиентский `id` в формате Guid-N + backdated `ts`) в ring-buffer/аудит-лог и broadcast'ит. Персистим **open + resolve** (progress/warning эфемерны — POST невозможен, пока бэк лежит); длительность — в expanded у resolve («Недоступен HH:mm:ss → HH:mm:ss (МСК) · N с»). Echo POST'а приходит с тем же `id` → дедуп на шине. Так инцидент **переживает reload**.
- **Точность начала**: по часам клиента, ± цикл WS-retry (~2 c). Более точный источник — `link_liveness.to_ts` осиротевшего интервала (`RecoverOpenIntervalsAsync` помечает краш `interrupted`) — будущий backend-side путь.
- **Ограничение**: мульти-вкладка → каждая вкладка заведёт свой инцидент (разные `correlationId`) → дубли в логе. Боль дедупа между клиентами — territory внешнего NC (см. выше).
