...
Статус: **DESIGN · СОГЛАСОВАНО** (модель зафиксирована 2026-07-24 по живому разбору Finam id=3; код —
следующим заходом, см. §8 «Что дорабатываем»). Это спецификация **connection-уровня**. Уровень
**данных/записи** (recording-лента оператора, `capture_liveness`, `md_trade`) — в 7h, см.
[../phase7h/incident.md](../phase7h/incident.md).
...
**Открытый вопрос хранения (решить при реализации):** как персистить **момент передачи владения** (жёлтое→
красное внутри одного инцидента). Варианты: (a) два смежных gap-сегмента под одним инцидентом; (b)
timestamp смены `owner` в записи. От этого зависит рендер тела ленты §7.
...
- [ ] **J1. `Degraded` = инцидент.** В `ConnectionManager.HandleLinkStateAsync` перенести `Degraded` из
...
- [ ] **J2. Один бит здоровья.** Инцидент открывается на любом уходе из `Live`, закрывается на возврате в
  `Live`. Свести server_status / стелс / supervisor к единому open/close по `_incidentSince`.
- [ ] **J3. Owner + handover.** Ввести владельца инцидента (`transaq`|`supervisor`), таймер `t`
  (`LinkRecoverGraceSeconds`=60 c). По истечении `t` в Degraded — супервизор форс-гасит сессию **особой
  причиной** (инцидент не закрывать) и берёт владение. Handover меняет тело ленты жёлтое→красное.
- [ ] **J4. NC-коды и дисциплина.** `connection.lost`(error/active) → `connection.recovering`(TRANSAQ,
  underway) / `connection.reconnecting`(supervisor, underway) → `connection.recovered`(ok/resolved) с
  expanded (кем + границы + длительность). Дедуп только по одному открытому инциденту, порогов нет.
- [ ] **J5. Прогресс-тик.** Наш таймер (супервизор) шлёт `recovering`/`reconnecting` с elapsed/попыткой,
  пока инцидент открыт.
- [ ] **J6. Хранение handover.** Персистить момент/владельца передачи (см. §6 открытый вопрос) — для рендера
  тела ленты и expanded.
- [ ] **J7. Connection-лента.** Красный стартовый маркер (1px) + зелёный конечный (1px); тело по owner
  (жёлтое TRANSAQ / красное supervisor); серое без маркеров для `disconnected`/`scheduled`.
- [ ] **J8. Конфиг.** `LinkRecoverGraceSeconds` в `OhsOptions`/`appsettings`.
...
2. **Хранение handover** (§6) — (a) два gap-сегмента vs (b) timestamp владельца.
