...
Нужен явный сигнал недоступности **client↔backend** — вид инцидента **`crash`** / подтип
`host_unavailable` (см. [incident.md](incident.md) §0a), отдельный от вида **`break`** (обрыв к бирже).
