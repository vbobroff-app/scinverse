...
Возврат в phase7. **Дерево descoped (2026-07-11):** отдельный компонент дерева и переключатель
«Список ↔ Дерево» не делаем — навигация по деривативам решается фильтрами-плашками (phase 7d),
а `/api/instruments/groups` используется как источник значений фильтра (серии по базовому активу).
Bulk-запись на узле реализуется как «Старт на серию» в плоском списке (по `underlyingId`/`expiration`).

Сначала спрашивай - потом делай
Stop & Commit полсле значимых доработок
Коомичу сам, ты пишешь commit message
