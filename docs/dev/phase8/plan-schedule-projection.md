...
### P2 — UI Schedule void mask (Connection ribbon) — **DONE**

| Шаг | Что | Критерий |
|-----|-----|----------|
| P2.1 | Toggle ⊥ SessionFilter | **DONE** — `showScheduleMask$` / Settings на ProviderCard |
| P2.2 | Void поверх трека; z-order §5.2 | **DONE** — `ConnectionRibbon` `.scheduleMask` |
| P2.3 | Tooltip «Окно простоя…» | **DONE** — `formatScheduleIdleTooltip` |
| P2.4 | Desired из резолвера connection (TZ, date>dow>main) | **DONE** — `scheduleVoidIntervals` / `enumerateDesiredWindows` |

Writers: клип через серверный `ScheduleCutter` в Write Gaps (**DONE**, phase 7h).
